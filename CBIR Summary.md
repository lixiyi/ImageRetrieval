...
* 表示方法（局部特征、全局特征）
  ...
  * 全局特征在诸如光照，形变，遮挡和裁剪这些情况下难以达到预想的效果，因而多用`局部特征`
  * 在图像检索领域，基于CNN提取的深度特征同样也表现出远优于传统特征的效果，所以`只研究深度特征`
  * 手工特征（传统特征）：
    * SIFT（局部）、GIST（全局）、HOG、HARR
...
* 存储方法（稀疏化）
  ...
  * 浮点特征
    * 特征编码：
      * BOF(Bag Of Feature, Bag Of Visual Words, BOW)、Fisher Vector、VLAD( Vector of Aggragate Locally Descriptor )
...
* 查找方法（通常：查找优化+向量优化）
  * 查找优化
    ...
    * NN 最近邻查找
      ...
      * 缺点：复杂度高
    * ANN 近似最近邻查找 
      * LSH、汉明编码、倒排多索引(IMI)（`将PQ应用于索引构建和查找的过程`）、针对深度特征的IMI（No-IMI）、HKM（Hierachical K-Means）、AKM（利用随机K-D树对K聚类中心索引）
...
* Fisher Vector
  ...
  * 核心思想：
    ...
    * GMM实际上也是一种聚类，考虑了特征点到每个聚类中心的距离，用所有聚类中心的线性组合去表示该特征点
...
* 乘积量化（PQ）
  * 核心思想：
    ...
    * 检索为避免穷尽搜索，采用ADC+倒排索引（IVFADC）,采用两级量化
      ...
      * 第二级量化是`对残差向量`使用乘积量化器PQ量化（NO-IMI是对残差向量再K-Means量化）
  * 优点：
    * PQ的量化空间非常大（质心数），可表示的向量之间的差异远远超过汉明空间能表示的向量差异。如质心数为$2^{64}$时，使用PQ方法，128维度向量的量化取值有$2^{64}$种
...
* 基于深度哈希特征的相似图像检索方法
  * 特征：基于深度神经网络全连接层输出的sigmoid特征的0.5 threshold映射后的`二进制特征`
  * 索引：多级检索方式。对所有商品定义`非常细粒度的类别`，细化到如‘运动鞋’、'高跟鞋'等。在深度神经网络进行特征学习的过程，`同时进行叶子类别分类的学习`，二者共享部分网络特征 (同时学习了索引的Key值)
  * 检索：
    * 查询图像仅在相同或相近的叶子类别中进行检索，大幅度减少了目标检索数据量
    * 对新的目标检索空间S下的数据进行hamming距离计算，采用穷尽法遍历，返回距离的最相近的Top list L
    * 对L中的数据结合商品属性标签进行重排序，返回最终的top相似结果
    * 商品属性包括不同的品牌、颜色等， 通过XGBoost训练得到商品属性预测模型。模型训练的输入是深度特征向量，文中提到使用了ResNet-50的pool5层输出向量。

### [Pinterest]()

* 公开时间早于ebay
* 提到大规模图像数据检索服务的`特征更新问题`
  * 特征更新：不影响现有服务运行的前提下高效生成`增量特征`的问题
  * 增量特征
    * 新增图像对应的特征
    * 算法模型更新带来的历史图像数据的特征更新
  * 解决方案：按照特征类型、版本和时间分开存储
    * 特征类型：全局特征、局部特征、深度特征、传统特征、浮点特征、二进制特征等
    * 版本对应算法模型更新：每个特征类型的每个版本对应一个feature store，每个feature store包含多个epoch，每个epoch对应新增图像数据的特征提取
    * 在这种存储方案下，对于每天新增图像数据，找到各特征类型的各版本，增加对应时间的feature epoch；对于新的特征或算法模型的更新，生成一个新的feature epoch，遍历所有历史图像数据，生成对应epoch。

### [Faiss](https://github.com/facebookresearch/faiss)

* 多媒体数据向量聚类和检索库 (FaceBook)
* 实现了各种聚类算法和经典的索引构建以及快速查找算法，包括上文介绍的IVF、IMI、PQ、LSH等
* Faiss不提供特征提取接口，即开发人员需要针对自己业务场景下的多媒体信息提取特征向量
* 当前版本的Faiss提供的方法更适合传统方法提取的特征向量
* 注意：各种索引构建和查找算法，是依赖于向量分布特性的，比如应用PQ算法时，需要保证划分后的向量空间是独立的，若划分后的向量强相关，那么查找效果可能会较差
